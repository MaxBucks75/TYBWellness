...
const STORAGE_KEY = "tybw_message_sent";
...
export default function ContactForm() {
  ...
  // Check localStorage on mount — soft guard against duplicate sends
  useEffect(() => {
    if (typeof window !== "undefined") {
      const flag = window.localStorage.getItem(STORAGE_KEY);
      if (flag === "true") setAlreadySent(true);
    }
  }, []);
  ...
  // ── Already sent (from a previous visit) ──────────────────
    ...
    return (
      <div className="bg-cream rounded-2xl p-10 text-center border border-sage-green/20">
        ...
        <p className="font-body text-warm-brown text-sm leading-relaxed max-w-md mx-auto">
          Thank you for reaching out. Dr. Bucks personally reviews every
          message and will respond as soon as possible. If you need to send
          another message, please call the office directly.
        </p>
      </div>
    );
  }
    ...
    >
      ...
      <div>
        <label htmlFor="phone" className="block font-body text-sm text-deep-green mb-2">
          Phone <span className="text-warm-brown-light">(optional)</span>
        </label>
        <input
          id="phone"
          type="tel"
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Share a little about what's bringing you here. There's no need to go into detail — just enough for Dr. Bucks to understand how she can help."
